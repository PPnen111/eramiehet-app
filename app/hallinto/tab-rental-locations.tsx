...
export default function TabRentalLocations({ clubId }: Props) {
  ...
  const selectCls = 'w-full rounded-lg border border-[#e0d8cc] bg-[#f5f0e8] px-3 py-2 text-sm text-[#1a1a1a] outline-none focus:border-[#2d6a2d]'
  ...
  return (
    <div className="space-y-4">
      ...
      {locations.length === 0 ? (
        <p className="text-sm text-[#888888]">Ei vuokrattavia kohteita.</p>
      ) : (
        <div className="rounded-2xl border border-[#e0d8cc] bg-white overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[600px] text-sm">
              <thead><tr className="border-b border-[#e0d8cc] bg-[#f5f0e8]">
                <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-[#2d6a2d]">Nimi</th>
                <th className="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-[#2d6a2d]">Tyyppi</th>
                <th className="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-[#2d6a2d]">Hyväksyjät</th>
                <th className="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-[#2d6a2d]">Tila</th>
                <th className="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-[#2d6a2d]">Toiminnot</th>
              </tr></thead>
              <tbody>
                {locations.map((loc) => {
                  const tb = typeBadge(loc.location_type)
                  return (
                    <tr key={loc.id} className="border-b border-[#e0d8cc]/30 hover:bg-white/[0.03]">
                      <td className="px-3 py-2.5 font-medium text-[#1a1a1a]">{loc.name}</td>
                      <td className="px-3 py-2.5 text-center"><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${tb.cls}`}>{tb.label}</span></td>
                      <td className="px-3 py-2.5 text-center text-[#1e3d1e]">{loc.approver_count}</td>
                      <td className="px-3 py-2.5 text-center">
                        <button onClick={() => void toggleActive(loc)} className={`rounded-full px-2 py-0.5 text-xs font-medium ${loc.is_active ? 'bg-[#1e3d1e] text-[#1a1a1a]' : 'bg-[#1e3d1e] text-[#4a4a4a]'}`}>
      ...
      {formOpen && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/50" onClick={() => setFormOpen(false)}>
          <div className="h-full w-full max-w-md overflow-y-auto bg-[#f5f0e8] p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-[#1a1a1a]">{editId ? 'Muokkaa kohdetta' : 'Lisää kohde'}</h2>
              <button onClick={() => setFormOpen(false)} className="rounded-lg p-1.5 text-[#2d6a2d] hover:bg-[#f0ebe3]"><X size={18} /></button>
            </div>
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-[#2d6a2d]">Perustiedot</h3>
              ...
              {f.location_type === 'ampumarata' && (
                <div className="rounded-lg border border-[#e0d8cc] bg-white/[0.03] p-3 space-y-2">
                  <p className="text-xs text-[#1e3d1e] font-medium">Miten ampumarata varataan?</p>
                  <label className="flex items-center gap-2 text-sm text-[#1a1a1a]"><input type="radio" checked={f.booking_unit === 'day'} onChange={() => setF((v) => ({ ...v, booking_unit: 'day' }))} /> Päivävaraus</label>
                  <label className="flex items-center gap-2 text-sm text-[#1a1a1a]"><input type="radio" checked={f.booking_unit === 'hour'} onChange={() => setF((v) => ({ ...v, booking_unit: 'hour' }))} /> Tuntivaraus</label>
                  {f.booking_unit === 'hour' && (
              ...
              <h3 className="text-sm font-semibold text-[#2d6a2d] pt-2">Varausten hyväksyjät</h3>
              <p className="text-xs text-[#888888]">Saavat ilmoituksen uusista varauksista ja voivat lähettää laskun varaajalle.</p>
              <div className="space-y-2">
                ...
                <button onClick={() => setApprovers((a) => [...a, { name: '', email: '', phone: '', is_primary: false }])} className="flex items-center gap-1 text-xs text-[#2d6a2d] hover:text-[#1e3d1e]"><Plus size={12} /> Lisää hyväksyjä</button>
              </div>

              <button onClick={() => void save()} disabled={busy || !f.name.trim()} className="w-full rounded-lg bg-[#1e3d1e] py-2.5 text-sm font-semibold text-white hover:bg-[#162d16] disabled:opacity-50 transition-colors">
